...
*/
void TiltCalculator::UpdateState(float pitchGyroMeasurement, float deltaT)
{
	/* Unbias our gyro */
	float unbiasedAngularRate = pitchGyroMeasurement - _AngularRateBias;
	 ...
	 */
	float Pdot[2 * 2] = {
		_Q_angle - _covariance[0][1] - _covariance[1][0],	/* 0,0 */
		- _covariance[1][1],		/* 0,1 */
		- _covariance[1][1],		/* 1,0 */
		_Q_gyro				/* 1,1 */
	};
	...
	AngularRateRadPerSec = unbiasedAngularRate;

	/*
	 * Update our angle estimate
	 * Angle += angle_dot * DeltaT
	 *       += (gyro - gyro_bias) * DeltaT
	 *       += q * DeltaT
	 */
	AngleRad += unbiasedAngularRate * deltaT;

	/* Update the covariance matrix */
	_covariance[0][0] += Pdot[0] * deltaT;
	_covariance[0][1] += Pdot[1] * deltaT;
	_covariance[1][0] += Pdot[2] * deltaT;
	_covariance[1][1] += Pdot[3] * deltaT;
}
...
void TiltCalculator::UpdateKalman(float measuredAngleRad)
{
	 ...
	 * term.
	 */
	float PCt_0 = C_0 * _covariance[0][0]; /* + C_1 * _covariance[0][1] = 0 */
	float PCt_1 = C_0 * _covariance[1][0]; /* + C_1 * _covariance[1][1] = 0 */
	...
	float t_1 = C_0 * _covariance[0][1]; /* + C_1 * P[1][1]  = 0 */

	_covariance[0][0] -= K_0 * t_0;
	_covariance[0][1] -= K_0 * t_1;
	_covariance[1][0] -= K_1 * t_0;
	_covariance[1][1] -= K_1 * t_1;
	 ...
	 * angle.
	 */
	AngleRad  += K_0 * angleError;
	_AngularRateBias += K_1 * angleError;
}
